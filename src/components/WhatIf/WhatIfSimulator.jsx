...
export default function WhatIfSimulator({ onApplySimulationToMap, initialZone }) {
  const [selectedPresetId, setSelectedPresetId] = useState('sim-add-2-amb-east');
  const [action, setAction] = useState('ADD');
  const [resourceType, setResourceType] = useState('ambulance');
  const [selectedZoneId, setSelectedZoneId] = useState(initialZone ? initialZone.id : 'zone-east');
  ...
  const handleRunSimulation = () => {
    ...
    setTimeout(() => {
      ...
      const preset = SIMULATION_PRESETS.find(p => p.id === selectedPresetId);
      if (onApplySimulationToMap) {
        onApplySimulationToMap({
          id: selectedPresetId,
          name: `${action} ${quantity} ${resourceType}(s) in ${targetZone.name}`,
          targetZoneId: targetZone.id,
          simulatedMarker: preset ? preset.simulatedMarker : {
            id: 'custom-sim',
            name: `Simulated ${resourceType}`,
